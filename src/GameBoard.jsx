...
const GameBoard = (props) => {
    ...
    const renderCreature = (c, isOpponent) => {
        ...
        return (
            <div key={c.instanceId} data-instance-id={c.instanceId}
                ...
                {!isOpponent && CardEngine.hasSummoningSickness(c, bz, mz) && <div className="zzz-overlay"><span className="zzz-icon">💤</span></div>}
            </div>
        );
    };
    ...
    const battleCtx = (e, c) => {
        ...
        if (!items.length) {
            const reason = hasSickness ? "Summoning sickness" : c.isTapped ? "Already tapped" : !canAtk ? "Can't attack" : !canAtkPlayer ? "Can't attack players" : "No targets";
